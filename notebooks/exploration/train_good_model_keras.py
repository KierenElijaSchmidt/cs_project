"""
Brain Tumor CNN Training Script with Weighted Recall (Good Architecture)
Trains the proven good architecture and saves in Keras format for Grad-CAM
"""
...
# Build model (GOOD ARCHITECTURE - same as train_improved.py)
...
# Dense layers (using Flatten, NOT GlobalAveragePooling)
x = layers.Flatten()(x)
...
# Compile model with weighted recall
model.compile(
    optimizer=keras.optimizers.Adam(learning_rate=0.001),
    loss="sparse_categorical_crossentropy",
    metrics=[
        WeightedRecall(class_weights=RECALL_WEIGHTS, name='weighted_recall'),
        "accuracy",
    ],
)
...
print("  Glioma (malignant):     3.0x weight - CRITICAL")
print("  Notumor (don't miss):   3.0x weight - CRITICAL")
