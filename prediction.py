...
CLIP_PCT = (1, 99)  # Percentile clipping from preprocessing
...
def load_model():
    ...
    if _model is None:
        ...
        # Get the serving signature (expects input_1 parameter)
...
def preprocess_image(image: np.ndarray) -> np.ndarray:
    ...
    2. Resize to 192x192
    3. Percentile clipping (removes outliers)
    4. Min-max normalization to [0, 1]
    5. Convert to uint8 then back to float32 [0, 255]

    The model has a built-in Rescaling(1./255) layer, so output must be in [0, 255] range.

    Args:
        image: Input image as numpy array (can be grayscale or RGB)

    Returns:
        Preprocessed image ready for model input (shape: (192, 192, 1), range: [0, 255])
    """
    ...
    # Percentile clipping (robust normalization - removes outliers)
    ...
    # Convert to uint8 (simulating saved preprocessed data)
    img = (img * 255.0).round().astype(np.uint8)

    # Convert back to float32 in [0, 255] range (model expects this)
    img = img.astype(np.float32)

    # Add channel dimension to make it (192, 192, 1)
...
def predict_single(image: np.ndarray) -> dict:
    ...
    # Predict - SavedModel signature requires named parameter 'input_1'
    output = model(input_1=tf.constant(batch, dtype=tf.float32))
    # Get the output tensor (key is 'output_0')
